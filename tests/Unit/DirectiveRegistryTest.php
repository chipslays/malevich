...
it('accepts a plain class string', function () {
    $builder = (new ClassListBuilder)->add('foo bar');

    expect((string) $builder)->toBe('foo bar');
});

it('accepts a conditional class array, à la Arr::toCssClasses', function () {
    $builder = (new ClassListBuilder)->add([
        'always-on' => true,
        'never-on' => false,
        'also-always-on',
    ]);

    expect((string) $builder)->toBe('always-on also-always-on');
});
...
it('merges several add() calls together', function () {
    ...
    expect((string) $builder)->toBe('foo bar baz');
});
...
it('ignores an add() call that compiles to an empty string', function () {
    ...
    expect((string) $builder)->toBe('foo');
});
